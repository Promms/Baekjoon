...
int Partition(int* scores, int left, int right){
    ...
    for(int k = left; k < right; k++){
        if(scores[k] <= pivot){
            i++; // 피봇보다 작은 수 +1
            Swap(&scores[i],&scores[k]); // 피봇보다 작은 수를 한 쪽으로 모음음
        }
    }
    Swap(&scores[i+1],&scores[right]); // 피봇보다 작은 수들을 scores[i]까지 모았으니 그 다음 위치에 피봇을 둔다다
...
void Quick_Sort(int* numbers, int left, int right){
    if(left < right){
        ...
        Quick_Sort(numbers,point_pivot+1,right); // 피봇보다 큰 부분을 정렬렬
    }
}
